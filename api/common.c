...
void Files2Subject(MIS * mis, char *outsubj)
{
    ...
    else if (mis->requested)
    {
        ...
        for (current = mis->requested; current; current = current->next)
        {
            ...
            if ((strlen(outsubj) + strlen(current->s) + strlen(temp) +
                 notfirst) < 72)
            {
                if (notfirst)   // First addition?
                    strcat(outsubj, " ");
...
int AnalyseTrail(char *s, unsigned len, MIS * mis)
{
    ...
    if (end == s)
        return 0;               // Not much, really :-)

    charptr = end - 1;

    // Now search back to find beginning of lines. And then check for SB
    // etc.
    ...
    while (!stophere)
    {
        ...
        if (charptr == NULL)
        {
            // It could be there is no '\r' anymore. Then this could be
            // the
            // first (or first and only) line in the message. And that
            // _can_ be
            // a VIA or SEENBY etc. if there is no 'normal' text at all in 
            // the
            // message! So don't just stop here! Check the first line
            // then!

            if (origptr && origptr == s) // We already were here.. Stop
                                         // now.
                break;
            else
                charptr = s;
        ...
        if (*charptr == '\0' || *charptr == '\x01') // Probably only if we 
                                                    // hit the start of
                                                    // buffer (charptr ==
                                                    // s)
            nowlast = charptr;
        ...
        *charptr++ = '\0';      // NULL out original '\r'.
        ...
        case 'S':
            ...
            {
                mis->seenby =
                    AddToStringList(mis->seenby, charptr + 9, NULL, 1);
                foundany = 1;
                end = nowlast;
            }
        ...
        case 'P':
            ...
            {
                mis->path =
                    AddToStringList(mis->path, charptr + 6, NULL, 1);
                foundany = 1;
                end = nowlast;
            }
            else
