...
void near Convert_Fmsg_To_Xmsg(struct _omsg *fmsg, XMSG * msg,
                               word def_zone);
...
//     mode = MOPEN_RDHDR;
    ...
    if (mode == MOPEN_CREATE)
    {
        ...
        if (msgnum == 0L)
        {
            ...
            if (!mh->locked)
            {
                ...
                if (fexist(msgname))
                {
                    /* If so, rescan the base, to find out which msg# it
                       is. */
    ...
    {
        if ((handle =
             sopen(msgname, (O_RDONLY | O_BINARY), SH_DENYNO,
                   S_IREAD | S_IWRITE)) == -1)
        {
            msgapierr = MERR_NOENT;
            return NULL;
    ...
    if (mode == MOPEN_CREATE)
    {
        ...
        /* If we're writing a new msg, this is easy -- just add to end of
           list */
        ...
        else
        {
            ...
            {
                /* Otherwise, we have to shift everything up by one since
                   we're * adding this new message inbetween two others.  */
...
dword SdmReadMsg(MSGH * msgh, MIS * mis, dword offset, dword bytes,
                 byte * text, dword clen, byte * ctxt)
{
    ...
    {
        lseek(msgh->fd,
              (dword) sizeof(struct _omsg) + msgh->rawclen + offset,
              SEEK_SET);
...
sword SdmWriteMsg(MSGH * msgh, word append, MIS * mis, byte * text,
        ...
        if (!append && (mis->seenby || mis->path || mis->via))
        {
            ...
            for (current = mis->seenby; current; current = current->next)
            {
                if (!(sq->type & (MSGTYPE_ECHO | MSGTYPE_NEWS)))
                    break;
                AddToString(&msgh->trail, &trailmax, &msgh->trailsize,
                            "\rSEEN-BY: ");
                AddToString(&msgh->trail, &trailmax, &msgh->trailsize,
                            current->s);
            }
...
int ReadHdrKludges(MSGH * msgh)
{
    ...
    if (read(msgh->fd, buffer, (unsigned)readlen) <
        (int)sizeof(struct _omsg))
    {
        msgapierr = MERR_BADF;
        free(buffer);
        return -1;
    ...
    ctrlbuf = (char *)CopyToControlBuf(ctl, &end, (unsigned *)NULL);
...
void SDM2MIS(struct _omsg *sdmhdr, MIS * mis)
{
    ...
    if ((!mi.useflags) &&
        (sdmhdr->attr & MSGCRASH) && (sdmhdr->attr & MSGHOLD))
    {
        mis->attr1 |= aDIR;
    ...
    if (sdmhdr->attr & (MSGFRQ | MSGURQ)) // extract requested files from
                                          // subject.
...
int SDMCalcTrailing(MSGH * msgh)
{
    ...
    if (msgh->bodylen < readlen)
        readlen = msgh->bodylen;
    ...
    if (lseek(msgh->fd, start, SEEK_SET) != start)
        return -1;
...
char *MIS2SDM(MSGH * msgh, MIS * mis, struct _omsg *sdmhdr, char *kludges)
{
    ...
    if (sq->type & (MSGTYPE_NET | MSGTYPE_MAIL)) /* Do we have to do it? */
    {
        if ((mis->attr1 & SQADDMASK1) || (mis->attr2 & SQADDMASK2))
        {
            ...
            {
                Attr2Flags(temp, (mis->attr1 & SQADDMASK1),
                           (mis->attr2 & SQADDMASK2));
                AddToString(&newkludges, &maxlen, &curlen, temp);
            }
        }

        // Now check for addition of INTL, FMPT, TOPT..
        if ((mis->destfido.zone != mi.def_zone
             || mis->origfido.zone != mi.def_zone)
            && !stristr(newkludges, "\x01INTL"))
        {
            sprintf(temp, "\x01INTL %hu:%hu/%hu %hu:%hu/%hu",
                    mis->destfido.zone, mis->destfido.net,
                    mis->destfido.node, mis->origfido.zone,
    ...
    else
    {
        ...
        tmdate = JAMsysLocalTime(&curtime); // We need something here for
                                            // Squish..
    }
...
#define ACCESS_FLAG 0x00
    ...
    rc = DosOpen(filename,      /* File path name */
                 ...
                 SHARE_FLAG | ACCESS_FLAG, 0L); /* No extended attributes */
