...
typedef struct
{                               /* MSGIDX.BBS structure definiton */
...
AREA *SelectArea(AREA * first, int pickonly, AREA * area_to_start)
{
  ...
  start:
    ...
    border =
        initbox(1, 0, rows + 2, 79, cfg.col[Casframe], cfg.col[Castext],
                SINGLE, NO, -1);
    printn(0, 0, cfg.col[Casbar],
                                   ...
                                   command back */
    {
        ...
        doredraw = (start == oldstart) ? 0 : 1; /* Need to repaint whole
                                                   screen? */
        oldstart = start;
        ...
        for (l = start; l < start + rows; curptr = curptr->next)
        {
            ...
            if (l == curline)
            {
                printn(l - start + 2, 1, cfg.col[Cashigh], temp, 78); /* Color 
                                                                         : 
                                                                         31 
                                                                       */
                highlighted = curptr;
                MoveXY(2, l - start + 3); // Position cursor for blind
                                          // users
            }
            else if (doredraw)
                printn(l - start + 2, 1, cfg.col[Castext], temp, 78);
        ...
        case cASscan:          /* ALT-S */
            oldstart = -1;      /* Force entire redraw of screen to update 
                                 * signs */
            ScanAreas(first, 0, 1);
            break;
        ...
        case cASscanpersonal:  /* ALT - P */
            if (TIMREGISTERED)
            {
                ...
                Message
                    ("Personal mailscan only available in registered version.",
                     -1, 0, YES);
...
void ScanAreas(AREA * first, int personal, int unscanned)
{
    ...
    msgbox =
        initbox(11, 9, 15, 69, cfg.col[Cpopframe], cfg.col[Cpoptext],
                SINGLE, YES, ' ');
    drawbox(msgbox);
    ...
    for (curarea = first; curarea; curarea = curarea->next)
    {
        ...
        if ((!(curarea->tagged)) &&
            ((cfg.mode == SHOWTAGGED) || (cfg.mode == SHOWNEWTAGGED)))
            continue;

        if (xkbhit() && (get_idle_key(1, GLOBALSCOPE) == 27)) /* User hit
                                                                 ESC */
            break;
        ...
        if (!(curarea->base & (MSGTYPE_SQUISH | MSGTYPE_HMB)))
        {
            ...
            if ((dword) curarea->lr == (dword) - 1L)
                curarea->lr = 0;
            if ((dword) curarea->lr == (dword) - 2L)
                curarea->lr = 0;
        }
        else
        {
    ...
    if (cfg.usr.hudsonpath[0] != '\0')
    {
        if (personal)
        {
            ...
            firstHMB = NULL;    // I think it's redundant, but what the
                                // heck...
        }
    }
#endif
...
int PersMsg(MSGA * areahandle, AREA * curarea)
{
    ...
    while (curno++ < MsgGetHighMsg(areahandle))
    {
        ...
        if (readresult != -1)
        {
            ...
            for (l = 0;
                 ((cfg.usr.name[l].name[0] != '\0') && (l < tMAXNAMES));
                 l++)
            {
                if (strcmpi(mis->to, cfg.usr.name[l].name) == 0)
                {
                    if (!
                        ((cfg.usr.status & SKIPRECEIVED)
                         && (mis->attr1 & aREAD)))
    ...
    for (this = firstHMB; this; this = this->next)
    {
        if (this->idx.board == board)
        {
            if (!areahandle)    // Don't open a second time!
            {
                if (!
                    (areahandle =
                     MsgOpenArea(curarea->dir, MSGAREA_CRIFNEC,
                                 curarea->base)))
                    return 0;
            }
...
int speedsearch(char c, AREA * first, int teller, int pickonly)
{
    ...
    for (teller = oldteller, current = first; current;
...
#define CHUNKSIZE 100
    ...
    while ((got = read(index, idxbuf, chunksize)) > 0)
    {
        ...
        while (iterations)
        {
            ...
            {
                for (loop = 0;
                     cfg.usr.name[loop].name[0] != '\0'
                     && loop < tMAXNAMES; loop++)
                {
                    if ((l->hash == cfg.usr.name[loop].hash) ||
                        (l->hash ==
                         (cfg.usr.name[loop].hash | 0x80000000Lu)))
                    {
    ...
    if (firstmsg)               /* personal msgs found */
    {
        if (!
            (areahandle =
             MsgOpenArea(curarea->dir, MSGAREA_CRIFNEC, curarea->base)))
            return 0;

        for (curmsg = firstmsg; curmsg && (ret != ESC);
             curmsg = curmsg->next)
        {
            if ((ret =
                 show_thismsg(areahandle, curarea, curmsg->uid)) == ESC)
                break;
        }
...
int show_thismsg(MSGA * areahandle, AREA * area, UMSGID uid)
{
    ...
    while ((command != ESC) && (command != NEXT) && (command == 0))
    {
        if ((persmsg = GetFmtMsg(curno, areahandle, area)) == NULL)
        {
            sprintf(msg, "Error reading msg #%d in %s", (int)curno,
                    area->tag);
            Message(msg, -1, 0, YES);
            putscreen();
            return NEXT;
        ...
        if ((persmsg->status & PERSONAL) &&
            (!((cfg.usr.status & SKIPRECEIVED)
               && (persmsg->mis.attr1 & aREAD))))
        {
...
int JAMpersonal(MSGA * areahandle, AREA * curarea)
{
    ...
    current =
        (long)curarea->lr -
        (long)((JAMDATA *) areahandle->apidata)->HdrInfo.BaseMsgNum;
    if (current < 0)
        current = 0;
    ...
    do
    {
        ...
        {
            sprintf(msg, "Seek too high - go away! (seek: %ld - len: %ld)",
                    (long)current * (long)sizeof(JAMIDXREC), len);
            Message(msg, -1, 0, YES);
        ...
        if (lseek(indexhandle,
                  ...
                  SEEK_SET) == -1)
        {
            sprintf(msg, "Error seeking JAM index (%s, errno %d)!",
                    curarea->tag, errno);
            break;
        }

        if ((bytesread =
             read(indexhandle, idxmem, 2000L * sizeof(JAMIDXREC))) == -1)
        {
            sprintf(msg, "Error reading JAM index (%s, errno: %d)!",
                    curarea->tag, errno);
            Message(msg, -1, 0, YES);
            break;
        }
        ...
        // Checked: check elements this read; current: keeps increasing
        // (realno)
        for (checked = 0; checked < total; checked++, current++, l++)
        {
            for (loop = 0;
                 cfg.usr.name[loop].name[0] != '\0' && loop < tMAXNAMES;
                 loop++)
            {
                if (l->UserCRC == cfg.usr.name[loop].crc)
                {
                    if (MsgUidToMsgn
                        (areahandle,
                         current +
                         ((JAMDATA *) areahandle->apidata)->HdrInfo.
                         BaseMsgNum, UID_EXACT) != 0)
                    {
                        curmsg = mem_calloc(1, sizeof(PERSMSG));
                        curmsg->uid =
                            current +
...
int quickscan(AREA * area)
{
    ...
    if (read(jhr, &jaminfo, sizeof(JAMINFO)) != sizeof(JAMINFO)) /* Corrupt? 
                                                                    Stop.. 
                                                                  */
    {
        close(jhr);
    ...
    /* Check if the area is 'contiguous', return 0 to do 'slow' scan if
       not */
    ...
    while (read(jlr, &lread, sizeof(JAMLREAD)) == sizeof(JAMLREAD))
    {
        ...
        {
            area->lr =
                (cfg.usr.status & JAMGETLAST) ? lread.LastReadMsg : lread.
                HighReadMsg;
...
void check_toidx(HMB_MSGTOIDX * thisentry, unsigned recno, int idx)
{
    ...
    for (l = 0; ((cfg.usr.name[l].name[0] != '\0') && (l < tMAXNAMES));
         l++)
    {
        if (strncmpi
            ((char *)thisentry + 1, cfg.usr.name[l].name,
             strlen(cfg.usr.name[l].name)) == 0)
        {
            ...
            if (lseek
                (idx, (long)((long)recno * sizeof(HMB_MSGIDX)),
                 SEEK_SET) != (long)((long)recno * sizeof(HMB_MSGIDX)))
            {
                Message("Error seeking Hudson index!", -1, 0, YES);
                continue;
            }

            if (read(idx, &thisone->idx, sizeof(HMB_MSGIDX)) !=
                sizeof(HMB_MSGIDX))
            {
...
void Analyse_HMB_Index(void)
{
    ...
    for (n = 0; n < total; n++, thismsg++)
    {
        ...
        {
            sprintf(msg,
                    "Illegal Hudson board number %d (msgnum: %d, recno: %d)!",
                    board + 1, thismsg->msg_num, n + 1);
            Message(msg, -1, 0, YES);
            continue;
        }
    ...
    for (; thisarea; thisarea = thisarea->next)
    {
        ...
        thisarea->lr =
            HMBareas[board].last ? HMBareas[board].last : HMBareas[board].
            lower;
        // Safe?
        if (thisarea->lr < 0)
            thisarea->lr = 0;
