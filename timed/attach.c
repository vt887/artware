...
int check_attach(MIS * mis, char *list, int copyfiles)
{
    ...
    while (file && ok)          /* Do all listed filespecs on subj line */
    {
        ...
        if ((strchr(filename, '*') != NULL) || (strchr(filename, '?') != NULL)) /* Wildcard 
                                                                                 */
        {
        ...
        else                    // We have a master list, we must add...
        {
            ...
            else                // Add multiple files
            {
                for (i = 0; thisone[i] != NULL; i++)
                {
                    ...
                    master[now] = NULL; // Zap out next el to indicate end 
                                        // of list. Realloc messed it up!
                }
    ...
    if (ok)
    {
        for (i = 0; master[i] != NULL; i++)
        {
            if (copyfiles && cfg.usr.localfiles[0] != '\0')
            {
                ...
                sprintf(temp, " ﷿ Copying %s to %s..", master[i],
                        filename);
                printeol(maxy - 1, 0, cfg.col[Cmsgbar], temp);

                if (CopyFile(master[i], filename, 0) != 0)
                {
                    sprintf(msg, "Error copying %s to %s!", master[i],
                            filename);
                    Message(msg, -1, 0, YES);
                }
                else
                    mis->attached =
                        AddToStringList(mis->attached, filename, NULL, 1);
            }
            else
                mis->attached =
                    AddToStringList(mis->attached, master[i], NULL, 1);
        }
    }
...
void writefa(MIS * mis, MSGA * areahandle, AREA * area, int exceptfirst)
{
    ...
    // JAM can handle as many files as you want, so this is the always
    // first..
    ...
    // Otherwise we are not handling JAM, so we need to split if the
    // subject
    // gets too long..
    ...
    while (((StrListLen(*files) > 71) && exceptfirst) ||
           (*files && !exceptfirst))
    {
        while (*files && ((StrListLen(tmplist) + StrListElLen(*files)) < 71) // -1 
                                                                             // for 
                                                                             // space!
            )
        {
            tmplist =
                AddToStringList(tmplist, (*files)->s, (*files)->pw, 0);
            temp = *files;
            *files = (*files)->next;
            temp->next = NULL;
            FreeStringList(temp);
...
int CopyFile(char *filename, char *tofilename, int delete)
{
    ...
    if ((out =
         sopen(tofilename, O_BINARY | O_CREAT | O_WRONLY, SH_DENYRW,
               S_IREAD | S_IWRITE)) == -1)
    {
        sprintf(msg, "Can't open %s for writing. (Errno %d)", tofilename,
                errno);
        Message(msg, -1, 0, YES);
        return -1;
    }
