...
#define PR_SQZ  6               /* For compressed mail files created by
                                   SQZ */
#define PR_UC2  7               /* For compressed mail files created by UC 
                                   II */
#define PR_RAR  8               /* For compressed mail files created by
                                   RAR */
...
typedef struct
{
    ...
    word dupentries;            /* Number of duplicate entries in
                                   ECHODUPE.GE */
    byte oldrcvdboard;          /* Where Rcvd netmail is moved to (0 =
                                   path) */
    byte oldsentboard;          /* Where Sent netmail is moved to (0 =
                                   path) */
    byte oldakaboard[OLDAKAS];  /* Netmail boards for AKAs */
    byte olduserboard[USERS];   /* Netmail boards for system users, 255 =
                                   use AKA board */
    ...
    byte gscolor[32];           /* GSETUP color set, See COLORSET
                                   structure */
    ...
    word userarea[USERS];       /* Netmail areas for system users, 0 =
                                   don't import, 65535 = use AKA area */
...
#define SECURITY  0x0002        /* Only accept mail from nodes in
                                   connections list */
...
#define TINYSB    0x0040        /* Tiny SEEN-BYs with only nodes in
                                   connections list */
...
#define FORMAT_JAM    3         /* Joaquim-Andrew-Mats message base
                                   proposal */
#define FORMAT_PCB    4         /* Joaquim-Andrew-Mats message base
                                   proposal */
#define FORMAT_SQUISH 5         /* Joaquim-Andrew-Mats message base
                                   proposal */
...
typedef struct
{
    ...
    word maxconnections;        /* Maximum number of entries in
                                   connections list */
}
AREAFILE_HDR;
...
typedef struct
{
    ...
    byte pkt_origin;            /* Address for the packet/Origin line
                                   (0-31) */
    dword seenbys;              /* Addresses (bits 0-31) to add to the
                                   SEEN-BY */
...
#define CONN_PAUSE     0x04     /* Temporary don't send this area to this
                                   node */
...
#define GE_ARCMAIL      0x0100  /* Use ARCmail 0.60 naming for out-of-zone 
                                   mail */
...
typedef struct
{
    ...
    word maxdays;               /* Maximum age of mail archive, 0 =
                                   Unlimited */
}
NODEFILE_GE;
